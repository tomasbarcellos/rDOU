...
#'
#' @export
extrair_normas <- function(arquivos, orgao_alvo, encodificacao = 'latin1') {
...
#'
#' @param lista_de_normas Lista de normas retornada pela função \code{extrair_normas}.
#'
...
estruturar_normas <- function(lista_de_normas) {
  ...
  normas <- tibble::tibble(
    numero = purrr::map_chr(lista_de_normas, pegar_numero),
    tipo = purrr::map_chr(lista_de_normas, pegar_tipo),
    orgao = attr(lista_de_normas, 'orgao'),
    texto = purrr::map_chr(lista_de_normas, paste, collapse = "\n"),
    promulgacao = attr(lista_de_normas, 'data_dou'),
    ementa = purrr::map_chr(lista_de_normas, pegar_resumo),
    titulo = purrr::map_chr(lista_de_normas, pegar_titulo),
    pagina = purrr::map_dbl(lista_de_normas, pegar_pagina, arquivos,
                            encodificacao),
    secao = as.integer(attr(lista_de_normas, 'secao'))
  )

  remover <- c(
    stringr::str_which(normas$titulo, "AS\\b"),
    stringr::str_which(normas$titulo,"OS\\b"),
    stringr::str_which(normas$titulo, desescapar("\\u00d5ES\\\\b"))
  ) %>% sort()
  ...
  res$numero <- as.integer(res$numero)
  res$secao <- as.integer(res$secao)
  res$pagina <- as.integer(res$pagina)
  res
}
